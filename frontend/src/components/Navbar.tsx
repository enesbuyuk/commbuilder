...
export default function Navbar({page}: {page: string}) {
    ...
    const [joinClubUrl, setJoinClubUrl] = useState('#');
    ...
    useEffect(() => {
        const fetchJoinClubUrl = async () => {
            try {
                const res = await fetch(`/api/external/contact/join-the-club`);
                if (res.ok) {
                    const data = await res.json();
                    if (data?.value?.[locale]) {
                        setJoinClubUrl(data.value[locale]);
                    }
                }
            } catch (error) {
                console.error('Failed to fetch join club URL:', error);
            }
        };
        ...
        <div className="w-11/12 mx-auto flex flex-wrap px-8 lg:px-4 py-4 items-center justify-between gap-8">
            ...
            <nav className={`grow fixed inset-0 bg-primary z-40 flex flex-col items-center justify-between lg:static lg:flex-row lg:bg-transparent transition-transform duration-300 ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'} lg:translate-x-0 py-5 lg:py-0`}>
                ...
                <div className="flex flex-col lg:flex-row items-center">
                    <LanguageSwitcher />
                    <a href={joinClubUrl} target="_blank" rel="noopener noreferrer" className="rounded-full border border-transparent bg-primary text-white hover:bg-secondaryDark text-sm px-6 py-3 transition-colors inline-flex items-center gap-2">
